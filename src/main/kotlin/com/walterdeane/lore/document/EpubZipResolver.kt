...
data class ResolvedUpload(val sourceType: SourceType, val suffix: String, val content: ByteArray)

/**
 * Resolves an uploaded file's declared extension to a [SourceType], with two zip-specific
 * fallbacks — browsers/OSes sometimes relabel an upload with a `.zip` extension:
 ...
 */
@Component
class EpubZipResolver {

    private val log = LoggerFactory.getLogger(EpubZipResolver::class.java)

    fun resolveUpload(declaredSuffix: String, content: ByteArray, filename: String): ResolvedUpload =
        when (declaredSuffix) {
    ...
    internal fun resolveZipUpload(content: ByteArray, filename: String): ResolvedUpload? {
     ...
     */
    internal fun findEpubMimetypePrefix(content: ByteArray): String? = try {
     ...
     */
    internal fun repackStrippingPrefix(content: ByteArray, prefix: String): ByteArray {
     ...
     */
    internal fun extractSingleWrappedDocument(content: ByteArray): Pair<String, ByteArray>? = try {
        ZipInputStream(content.inputStream()).use { zip ->
            var entry = zip.nextEntry
